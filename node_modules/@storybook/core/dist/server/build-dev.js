...
var _devServer = _interopRequireWildcard(require("./dev-server"));
...
function outputStartupInformation(options) {
  ...
  console.log((0, _boxen.default)(_commonTags.stripIndents`
          ...
          ${_chalk.default.gray(_commonTags.stripIndents`
          ${_chalk.default.underline((0, _prettyHrtime.default)(managerTotalTime))} for manager and ${_chalk.default.underline((0, _prettyHrtime.default)(previewTotalTime))} for preview`)}
...
async function buildDevStandalone(options) {
  try {
    ...
    const storybookMiddleware = await (0, _devServer.default)(options);
    ...
    const [{
      ...
      managerTotalTime,
      previewTotalTime
    }, updateInfo] = await Promise.all([_devServer.webpackValid, updateCheck(version), serverListening]);
